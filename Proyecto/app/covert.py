...
# def ocultar_mensaje_imagen(image_path, message):
...
#                     pixel[color_index] = (pixel[color_index] & 0xFE) | int(message_len[pixel_index])
#                     pixel_index += 1
...
#     # Paso 1: Preparación
#     mensaje_secreto = "Este es un mensaje muy secreto."
...
def ocultar_mensaje_imagen(image_file, message):
    ...
    for i in range(width):
        for j in range(height):
            pixel = list(pixels[i, j])  # Convert the pixel to a mutable list
            
            for color_index in range(3):  # Iterate over R, G, B channels
                if pixel_index < message_len:
                    # Modify the least significant bit (LSB) of the color channel
                    pixel[color_index] = (pixel[color_index] & 0xFE) | int(message_bin[pixel_index])
                    pixel_index += 1
    ...
    # Create a new ImageFile instance and save the modified image
    hidden_image = image()
    hidden_image.length = message_len
    hidden_image.image.save("hidden_image.png", ContentFile(output.read()), save=True)
    output.close()

def extraer_mensaje_imagen(image_path, longitud_bits):
    img = Image.open(image_path)
    pixels = img.load()
